...
**최종 목표:** LangChain · LangGraph · Deep Agents를 소스 코드 수준까지 이해해 오픈소스 PR 기여가 가능한 지식을 갖춘다.
**학습 축:** `docs/wiki/_book_roadmap.md` — 100개 섹션을 순서대로 완주한다. 각 섹션 = 블로그 글 1편 = 사이클 1회.
**AI 역할:** 사용자가 요청한 섹션에 필요한 위키·소스를 준비한다. 요청 없이 먼저 행동하지 않는다.
