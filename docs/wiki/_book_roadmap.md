...
각 섹션 제목 뒤의 기호는 블로그 작성 전 위키 준비 상태를 나타낸다.

| 기호 | 의미 | 판정 기준 |
|------|------|-----------|
| ✅ | 준비됨 | 관련 위키 페이지 존재 + `status: verified` |
| 🟠 | 소스 있음 | 관련 위키 페이지 존재 + `status: partial` 또는 verified 페이지가 일부 커버 |
| 🟡 | 초안 | 관련 위키 페이지 존재 + `status: draft` |
| ❌ | 없음 | 관련 위키 페이지 없음 |
...
- [ ] **14.2** InjectedState / InjectedStore — ToolNode의 런타임 주입 패턴 ✅
  - 위키: [[LangGraph ToolNode flow]], [[Store]]
