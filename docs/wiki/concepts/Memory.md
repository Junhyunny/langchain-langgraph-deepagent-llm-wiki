...
*상태: LangGraph Store 인터페이스 추가됨. LangChain 섹션 소스 필요 (ConversationBufferMemory deprecated 여부 미확인).*
...
- LangGraph에서 `MemorySaver`와 `InMemorySaver`는 동일한 클래스인가, 다른 클래스인가? — 소스에서 `InMemorySaver`만 확인됨. `MemorySaver`는 alias 또는 구버전 이름일 가능성. 직접 import 경로 확인 필요. Source: `langgraph-source-checkpoint-runtime-2026-05-20`
