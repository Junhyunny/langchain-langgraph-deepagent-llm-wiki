...
- [ ] issue #5225 회귀 테스트 직접 작성: Pydantic BaseModel + `default_factory` + reducer 조합 테스트
  - 패턴 참고: `test_pregel_checkpoint_map.md`, `test_pregel_interrupt_map.md`
  - 예상 위치: `reproductions/langgraph_pydantic_default_factory/test_regression.py`
  - 검증할 케이스: `invoke({})`, `invoke(State())`, `invoke({'variable': [...]})`, checkpointer resume
- [ ] 테스트가 없는 동작이나 엣지 케이스를 스스로 발견하기
