...
- [x] Deep Agents: `SubAgentMiddleware` task tool + compiled subagent state isolation 직접 확인 ✅ (2026-05-30, `examples/deepagents_core/04_subagent_middleware_task_tool.py`)
- [x] Deep Agents: 단일 `AIMessage`의 다중 `task` tool call 병렬 실행과 reducer 병합 순서 확인 ✅ (2026-05-30, `examples/deepagents_core/05_subagent_parallel_tasks.py`)
...
- [x] LangGraph: `ToolNode` 직접 실행 + `wrap_tool_call` request override 확인 ✅ (2026-05-28, `examples/langgraph_core/08_toolnode_direct.py`)
- [x] LangGraph: `ToolNode`의 `Command(update/goto)` 반환 동작 직접 확인 ✅ (2026-05-29, `examples/langgraph_core/09_toolnode_command_outputs.py`)
- [x] LangGraph: child `ToolNode`의 `Command.PARENT + Send` fan-out 확인 ✅ (2026-05-30, `examples/langgraph_core/10_toolnode_parent_command_send.py`)
