...
→ `examples/langchain_core/`

| 파일 | 확인할 것 |
|------|----------|
| `01_lcel_chain.py` | `RunnableLambda`, `\|` → `RunnableSequence`, `RunnableParallel` |
| `02_tool_calling.py` | `@tool` → `StructuredTool`, args_schema JSON, ToolCall → ToolMessage 연결 |
| `03_create_react_agent.py` | `create_agent` → `CompiledStateGraph`, 멀티턴 체크포인팅 |
...
- `tool.invoke(ToolCall객체)` 직접 호출은 실패 → `tool.invoke(tool_call["args"])` 방식이 올바름.
- `FakeMessagesListChatModel.bind_tools()` → `NotImplementedError` 발생. `create_agent`는 내부적으로 `model.bind_tools(tools)` 호출함.
...
- `RunnableConfig.configurable` 필드를 통해 실행 시간 설정을 주입하는 방법은? — Source: `langchain-source-runnable-2026-05-23`
