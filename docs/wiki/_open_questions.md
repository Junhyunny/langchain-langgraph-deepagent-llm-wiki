...
- `wrap_tool_call`이 `Command`를 반환할 때 어떤 상태 변화가 가능한가? — **부분 검증됨**: `ToolNode`는 wrapper 반환값이 `Command`일 수 있고 `_combine_tool_outputs()`에서 `Command.PARENT + Send`를 병합한다. 실제 graph routing 결과는 별도 실험 필요. Source: `langgraph-prebuilt-tool-node-2026-05-27`, [[2026-05-28 langgraph toolnode direct]]
