...
- `wrap_tool_call`이 `Command`를 반환할 때 어떤 상태 변화가 가능한가? — ✅ **검증됨** (2026-05-29): current graph 기준 `Command.update`는 compiled graph state에 적용되고, `Command.goto`는 정적 edge 없이 대상 노드로 라우팅할 수 있다. 단, matching `ToolMessage`가 없으면 `ValueError`. `Command.PARENT`는 별도 확인 필요. Source: [[2026-05-29 langgraph toolnode command outputs]]
